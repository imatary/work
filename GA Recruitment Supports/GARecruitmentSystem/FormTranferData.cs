...
{
    public partial class FormTranferData : XtraForm
    {
        ...
        private void GetResultsByDate(string dateSearch)
        {
            var result = _resultService.GetResults(Ultils.ConvertStringToDateTime(dateSearch));
            if (result != null)
            {
        ...
        {
            splashScreenManager1.ShowWaitForm();
            GetResultsByDate(txtSearchDate.Text);
            splashScreenManager1.CloseWaitForm();
        }
    }
}