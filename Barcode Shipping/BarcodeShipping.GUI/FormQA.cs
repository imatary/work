                ...
                || !string.IsNullOrEmpty(txtJudge.Text))
            {
                ...
                if (operationId == 1)
                {
                    if (!_iqcService.CheckPcbExits(txtProductionID.Text))
                    {
                        ...
                        if (log == null)
                        {
                            if (logs.Any())
                            {
                                ...
                                if (firstPcbInBox != null)
                                {
                                    ...
                                    {
                                        SetErrorStatus(true, "NG", string.Format("Error {0} !\nPCB [{1}]\nnày khác với các PCB trong Box [{2}].\nVui lòng kiểm tra lại!", str2, txtProductionID.Text, boxId));
                        ...
                        {
                            SetErrorStatus(true, "NG", string.Format("PCB [{0}] đã được bắn trong Box rồi.\nVui lòng kiểm tra lại", txtProductionID.Text));
                            ResetControls();
                            var refeshData = _iqcService.GetLogs(boxId).ToList();
                            gridControlData.Refresh();
                            gridControlData.DataSource = refeshData;
                    ...
                    else
                    {
                        ...
                        SetErrorStatus(true, "NG", string.Format("PCB [{0}] đã được bắn trước đó." +
                                                                 "\nVui lòng kiểm tra lại\n" +
                                                                 "Box ID: {1}" +
                                                                 "Operator: {2}" +
                                                                 "Date Check: {3}", 
                                                                 txtProductionID.Text, log.BoxID, log.OperatorCode, log.DateCheck));
                        ResetControls();
                    }
                }
